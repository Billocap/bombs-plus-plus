...
MenuOption::MenuOption(std::string label, IMenuAction *action)
{
  this->label = label;
  this->action = action;
}
...
void Menu::add_option(std::string label, IMenuAction *action)
{
...
void render_menu(Menu *menu, RenderEvent *event)
{
  ...
  for (auto option : menu->options)
  {
    ...
    auto child_event = new RenderEvent(event->width, event->height, event->x, y);
